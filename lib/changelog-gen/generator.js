...
const formatIssues = (owner, repo, issues) => {
  ...
  issues.forEach(i => {
    if (!i.repo) {
      ...
      s += ` [\#${i.id}](${config.ghRepoUrl}/${owner}/${repo}/issues/${i.id})`
    } else {
      // External issue
      s += ` [${i.repo}\#${i.id}](${config.ghRepoUrl}/${i.repo}/issues/${i.id})`
    }
  })
  return s
}
...
const groupByLabel = prs => {
  ...
  for (let pr of prs) {
    ...
    if (pr.labels.length === 0) {
      delete pr.labels
      if (!parts[config.defaultLabel]) {
        parts[config.defaultLabel] = []
      }
      parts[config.defaultLabel].push(pr)
    } else {
      if (pr.labels.includes(config.breakingChangesLabel)) {
        hasBreakingChanges = true
      }

      if (pr.labels.includes('changelog:'))

      // Exclude all changelog:exclude labels
      pr.labels = pr.labels
        .filter(e => e !== config.excludeLabel)
      ...
      pr.labels = pr.labels
        .filter(e => e !== config.complementLabel)
...
const sortByLabel = (prs) => {
  ...
  for (let label of config.labelsPriority) {
    if (prs[label]) {
...
module.exports = {
  generate: Bluebird.coroutine(function* (owner, repo, tag, currentVersion, prs) {
    const
      date = new Date()
    ...
    yield verifyBreakingChange(hasBreakingChanges, currentVersion, tag)

    changeLog += `### [${tag}](${config.ghUrl}/${owner}/${repo}/releases/tag/${tag}) (${date.getFullYear()}-${(month < 10 ? '0' + month : month)}-${date.getDate()})\n\n`
    changeLog += `[Full Changelog](${config.ghUrl}/${owner}/${repo}/compare/${currentVersion}...${tag})\n`

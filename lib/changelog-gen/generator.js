...
const groupByLabel = prs => {
  ...
  prs
    .filter(pr => pr.labels.indexOf(config.labels.exclude) === -1)
    .forEach(pr => {
...
const verifyBreakingChange = (currentVersion, tag) => {
...
module.exports = class Generator {
  ...
  generate(currentVersion, prs) {
    ...
    return verifyBreakingChange(currentVersion, this.tag)
      .then(() => {
        changeLog += `# [${this.tag}](${config.github.url}/${this.owner}/${this.repo}/releases/tag/${this.tag}) (${date.getFullYear()}-${(month < 10 ? '0' + month : month)}-${date.getDate()})\n\n`
        // changeLog += `[Full Changelog](${config.github.url}/${this.owner}/${this.repo}/compare/${currentVersion}...${this.tag})\n\n`

        changeLog += '### Compatibility\n\n'
        changeLog += '| Kuzzle | Proxy |\n'
        changeLog += '|--------|-------|\n'
        changeLog += `| ${compat.KUZZLE_VERSION} | ${compat.PROXY_VERSION} |\n`

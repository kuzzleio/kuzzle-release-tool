...
module.exports = class Changelog {
  ...
  async generate() {
    ...
    console.log(`\x1b[32mNew repository tag: ${this.tag}\x1b[0m`);
  ...
  forceTag(tag) {
    if (!semver.valid(tag)) {
      throw new Error(`Tag ${tag} is not a valid semver tag.`);
    }
    this.bypassTag = true;
    this.tag = tag;
  }
  ...
  _getPRs() {
    const commands = [
      {command: 'git fetch', ignoreError: true},
      `git log --abbrev-commit origin/${this.vinfo.release}..origin/${this.vinfo.development}`
    ];

    return exec(this.dir, commands)
      .then(stdout => {
        return Bluebird.map(stdout.split('\n'), pr => {
          // Merge commits
...
\x1b[0m`);
    ...
    return semver.inc(this.currentVersion, releaseType);
  }

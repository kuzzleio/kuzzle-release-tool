...
module.exports = {
  createProposalBranch (tag) {
    return exec(`cd kuzzle-test-environment && git checkout -b ${tag}-proposal`)
  }
  , pushProposalBranch (tag) {
    return exec(`cd kuzzle-test-environment && git commit -am "Update travis.yml for release" && git push origin ${tag}-proposal`)
  }
  , deleteProposalBranch (tag) {
    return exec(`cd kuzzle-test-environment && git push origin --delete ${tag}-proposal ; git checkout master ; git branch -D ${tag}-proposal`)
  }
  , writeMatrix () {
    let matrix = {}

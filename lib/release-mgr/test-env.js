...
module.exports = {
  ...
  , deleteProposalBranch (tag) {
    return exec(`cd kuzzle-test-environment && git branch | grep ${tag}-proposal`)
      .then(() => exec(`cd kuzzle-test-environment && (git push origin --delete ${tag}-proposal ; git checkout master ; git branch -D ${tag}-proposal)`))
  ...
  , streamLog (ghToken, tag) {
    return new Promise((resolve, reject) => {
      const spinner = cliSpinner()

      async.retry({times: config.travis.totalRetry, interval: config.travis.retryInterval}, getBuildFromTravis, (err) => {
        if (err) {
          return reject(err)
        }
        exec(`cd kuzzle-test-environment && travis login --github-token ${ghToken} && travis branches | grep ${tag} | awk '{gsub(/#/, ""); print $2}'`)
          .then(build => {
